...
// Data
   ...
   //Structure to hold the details of a persons
    ...
    Part 1 Task 4. Implement the three conditions below.
      4.1 If the number of tokens remaining with the bidder is < count of tokens bidded or there are no tokens remaining with the bidder, revert.
      4.2 If the id of the item for which bid is placed, is greater than 2, revert.

    Hint: "tokenDetails[msg.sender].remainingTokens" gives the details of the number of tokens remaining with the bidder.
    */
    
        // ** Start code here. 2 lines approximately. **/
        require(_count > 0 && _count <= tokenDetails[msg.sender].remainingTokens, "Invalid token count");
        require(_itemId < 3, "Invalid item Id");
    ...
    /*Part 1 Task 5. Decrement the remainingTokens by the number of tokens bid and store the value in balance variable.
        ...
        // ** Start code here. 1 line approximately. **
        uint balance= 0;
        balance = tokenDetails[msg.sender].remainingTokens - _count;
        //** End code here. **
  ...
  modifier onlyOwner {
    // ** Start code here. 2 lines approximately. **
        require(msg.sender == beneficiary, "Only owner can reveal the winners");
        _;
    //** End code here. **
    }
